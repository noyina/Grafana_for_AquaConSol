...
export class GeomapPanel extends Component<Props, State> {
  ...
  optionsChanged(options: GeomapPanelOptions) {
    ...
    if (options.controls !== oldOptions.controls) {
      this.initControls(options.controls ?? { showZoom: true, showAttribution: true, showBlendmap:true });
    }
  }
  ...
  /* let feature = this.map?.forEachFeatureAtPixel(evt.pixel,function(feat,layer){
    return feat;
   });
  ...
  const urlsel = window.location.href;
    ...
    let openerString = "";
    if(urlsel.includes("&viewPanel")){
      openerString = map[0]+ "?orgId=2&viewPanel="+this.state.ttip?.data?.fields.at(3)?.values.get(feature?.getProperties()['rowIndex']) ;
    }
    else{
      openerString = dash[0]+"/"+dash[1]+"/"+dash[2]+"/"+dash[3]+"/"+dash[4]+"/"+dash[5]+"?orgId=2" + "&viewPanel=" + this.state.ttip?.data?.fields.at(3)?.values.get(feature?.getProperties()['rowIndex']) ;
    }
    
    console.log(openerString);
    window.open(openerString);
    */
  ...
  initControls(options: ControlsOptions) {
    ...
    if(options.showMap){
     ...
     const staticImageLayer = new ImageLayer({
      source: new Static({
        url: 'https://static.uni-graz.at/fileadmin/_processed_/f/f/csm_Strategy_3dcd0a96ff.jpg',
        imageExtent: [15.45,46.971,16.45,48.971],
        projection: 'EPSG:3857',
    }),
    zIndex: 200,
     });
     this.map?.addLayer(staticImageLayer);
      window.console.log(options.showMap);
    }
